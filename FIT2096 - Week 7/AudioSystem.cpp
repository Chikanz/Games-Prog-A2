...
AudioClip* AudioSystem::Play(const char* filepath, bool startPaused)
{
	...
	if (soundToPlay)
	{
		...
		if (DidSucceed(m_audioEngine->playSound(soundToPlay, NULL, startPaused, &channel)))
		{
			...
			AudioClip* c = new AudioClip(channel);
			Clips.push_back(c);
			return c;
		}
	}
...
void AudioSystem::Update(float simSpeed)
{
	...
	{		
		if(Clips[i]->TimeShift) 
			Clips[i]->SetPitch(MathsHelper::RemapRange(simSpeed, 0.013f, 0.8f, 0.5f, 0.9f));
	}
}
